...
- If the work needs a change to a protected path or to `.github/workflows`, stop and return `blocked`. Do not make the change. The protected paths are listed in `.cumin/config.toml` of the repository. The default list is `.cumin/`, `CLAUDE.md`, `AGENTS.md`, and `.claude/`.
