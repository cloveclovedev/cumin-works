...
- リポジトリごとに、続けて失敗した回数と、その理由を数える。3回目に1回だけOwnerに知らせ、そのリポジトリの定期確認が成功するまで、それ以上は送らない。理由が変われば別の問題なので、数え直す。
...
- 通知を出すかどうかは、直前の定期確認が読み取った、そのリポジトリの設定で決める。まだ一度も読めていないときと、リポジトリのファイルが失敗の原因のときは、Hostの設定で決める。リポジトリが、自分で起こした失敗の通知を止められないようにするためである。
