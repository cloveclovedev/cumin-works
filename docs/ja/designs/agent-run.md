...
- 実行の最初に出る `init` のイベントで、Agentが作業場所の外の文脈を読み込んでいないことを確かめる。確かめる項目は、`plugins` と `mcp_servers` (空でなければ異常)、`memory_paths` (あって、作業場所の外のパスを指していれば異常) である。`--setting-sources project` を付けると `plugins` と `mcp_servers` は空になり (実測 27)、自動メモリを切ると `memory_paths` は項目ごと現れない (実測 28。2026-09-21 の最小の実機実行でも同じ)。
