...
- オプション (公式: Run Claude Code programmatically、CLI reference):
  - `--append-system-prompt <roleの指示>`: roleの指示はシステムプロンプトの末尾に足す。Claude Code既定のシステムプロンプト (ツールの使い方、リポジトリの `CLAUDE.md`) はそのまま活かす。
    - roleの指示は、`roles/<role>.md`、そのroleのdisciplineのファイル (`disciplines/<discipline>/<role>.md`)、平易な英語の決まり (`templates/writing-rules.md`) を、この順に連結したものである。disciplineのファイルがないroleは、残りの2つを受け取る。平易な英語の決まりは全ての文章に効くので、指示に入れる。合成の決まりは [Agentに共通の要件](../requirements/agents/common.md) の「指示の合成」にある。続きの依頼でも同じ指示を渡すので、依頼の種類に依存することは、指示ではなく依頼文に書く。
...
- 異常終了 (プロセスの失敗、`result` がない、`is_error`、結果がスキーマに合わない、実行時間の上限、起動の記録にユーザの文脈がある) のとき、呼び出し処理は同じ依頼を1回だけやり直す。作業場所は同じままである。途中までの作業がworktreeに残っているので、捨てない。
- やり直しは、新しいセッションで始める。`--resume` を付けない。
- 理由は3つある。異常終了の種類の多くは、セッションの番号を返さない (プロセスの失敗、`init` のイベントがない) か、同じ結果を繰り返す (起動の記録にユーザの文脈がある、環境の誤り)。実行時間の上限で止めた実行を再開すると、長い文脈を読み直す分だけ利用枠を余計に使い、また上限に当たりやすい。判定が1つで済み、種類ごとの分岐が要らない。
...
- 2回目も異常終了なら、呼び出し処理がIssueをOwnerに戻す ([定期確認の設計](poll.md) の「うまくいかなかったときに、Ownerに戻す道」)。
- やり直した回数は、1回の着手の中の値である。手元のファイルにも、サービスの状態にも持たない。Ownerが `cumin/status/ready` を付け直すと、I1が新しいセッションで始め、回数は0から数え直す (Issueのラベルと状態遷移の「Agentのセッションの扱い」)。
