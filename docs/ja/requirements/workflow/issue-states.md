...
| ラベル | 付く対象 | 意味 | 付ける人 |
|---|---|---|---|
| `cumin/type/requirement` | 要求Issue | これは要求Issueである。付けることが、要求の提出の合図になる | Owner |
| `cumin/status/ready` | 実装Issue、要求Issue | Ownerが「進めてよい」と合図した | Owner |
| `cumin/status/planning` | 要求Issue | Chief Engineerが分割している | cumin |
| `cumin/status/implementing` | 実装Issue、要求Issue | 実装Issueでは、Implementerが作業している (修正を含む)。要求Issueでは、sub-issueの実装が進んでいる | cumin |
...
|---|---|---|---|---|
| R1 | 要求Issueに `cumin/status/planning` を付け、Chief Engineerに分割を依頼する | 定期確認: 開いていて、`cumin/type/requirement` が付き、sub-issueがなく、状態ラベルがないか `cumin/status/ready` が付いた要求Issueがある | AIリソースに空きがある | — |
...
| R3 | 要求Issueのラベルを `cumin/status/implementing` に替える | 定期確認: sub-issueのどれかに `cumin/status/ready` が付いた | — | — |
| R4 | 要求Issueのラベルを `cumin/status/awaiting-owner-review` に替え、Ownerに「受け入れ可能になった」と通知する | 定期確認: sub-issueが全て閉じた | sub-issueが1つ以上ある | — |
| R5 | (何もしない。完了) | Ownerが要求Issueを閉じた | — | — |

差し戻しのとき、Ownerはsub-issueを追加して `cumin/status/ready` を付ける。これでR3が再び成り立ち、追加分が閉じるとR4が再び成り立つ。

分割に失敗して `cumin/status/awaiting-owner-decision` になったとき、Ownerは要求Issueを直し、要求Issueに `cumin/status/ready` を付ける。これでR1が再び成り立つ。`cumin/type/requirement` は要求Issueである印なので、外さずに付けたままにする。Ownerの「進めてよい」の合図を、実装Issueと同じ `cumin/status/ready` に揃えるため、この形にしている。
...
| I2 | 必須のcheckの完了を待ち始める | 実行終了: Implementerの実行が終わり、結果が `done` | このIssueを閉じるPull Requestが開いている。ブランチの先頭のコミットがpushされている | 結果が `blocked`、またはPull Requestがないなら `cumin/status/awaiting-owner-decision` に替えて通知する。異常終了なら1回だけやり直し、それでも駄目なら同じ扱いにする |
| I3 | ラベルを `cumin/status/reviewing` に替え、Reviewerにレビューを依頼する | 定期確認: 必須のcheckが、Pull Requestの先頭のコミットで全て通った。必須のcheckが1つもなければ、すぐに通ったとみなす | レビューのラウンドが上限 (3) に達していない | — |
| I4 | ラベルは `cumin/status/implementing` のまま、失敗したcheckの内容を添えてImplementerに修正を依頼する | 定期確認: 必須のcheckのどれかが失敗した | checkの修正依頼が上限 (3回) に達していない | 上限に達したら `cumin/status/awaiting-owner-decision` に替えて通知する |
...
Implementerが `blocked` を返したとき (I2) は、やり直さずに、1回目から `cumin/status/awaiting-owner-decision` に替える。実装に必要な要件が足りない、という場合が多いためである。Ownerは実装Issueか、その上の要求Issueを書き直して進められる状態にしてから、実装Issueに `cumin/status/ready` を付け直す。
...
|---|---|---|---|
| Q1 | 新しい着手 (R1、I1) を止め、Ownerに通知する。実行中のIssueは最後まで進める | 実行終了: どれかのAgentの実行結果に含まれる5h枠またはweekly枠の使用率が、しきい値を超えた | この枠でまだ通知していない |
...
- 辻褄の合わないIssueの回収。cuminが再起動すると、`cumin/status/planning`、`cumin/status/implementing`、`cumin/status/reviewing` のまま、実行中のAgentがいないIssueが残りうる。将来は、ラベルとcuminの動作状態を突き合わせて、適切な状態まで戻す機能を作る。v0.1では、OwnerがそのIssueに `cumin/status/ready` を付け直せば、I1により続きから再開する。
...
- `GET /repos/{owner}/{repo}/rules/branches/{branch}` をGitHub Appのtokenで呼べるか、どの権限が要るか。公式ドキュメントの該当ページには記載がなかった。
