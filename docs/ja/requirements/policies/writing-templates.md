...
- 位置づけ (製品のどの部分を変えるか) は、Plannerが実装Issueの「Where this fits」で決める。粒度は、読み手が図の1か所を指せること。Pull Requestは同じ見出し「Where this fits」にそれをそのまま写し、実装で変わったときだけ書き直す。Pull Requestの図は、そのPull Requestが変えた設計文書のSVGである。図がない部分は、実装Issueの完了条件に「設計文書にこの部分の図を足す」と書く。
