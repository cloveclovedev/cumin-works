...
- Under "What", give the same information that the diff of a design note gives: how it worked before, what this pull request changes, and the approach. Write it as "Before" and "After", each one to three lines. Reuse the "Where this fits" of the issue as it is; rewrite it only when the implementation moved the change to another place. The diagram is the SVG of the design document that this pull request changed, as an image at the commit of this branch; when the pull request changed no diagram, the image of the issue. An identifier never stands alone; add its meaning.
...
```markdown
## What
<!-- One or two sentences, written as an order: "Add ...". Then the "Where this fits" of the issue (the image and the sentence), and "Before" and "After", one to three lines each: how it worked, what changes, and the approach. -->
Where this fits: <!-- from the issue -->
Before: ...
After: ...
...
Where this fits: the failure branches of the I2 verification, in the diagram below (changed by this pull request).
